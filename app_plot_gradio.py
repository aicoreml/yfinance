...
def plot_stock(ticker="AAPL", start_date="2025-01-01", end_date="2025-03-13"):
    stock_data = yf.download(ticker, start=start_date, end=end_date)

    plt.figure(figsize=(10, 6))
    plt.plot(stock_data['Close'], label='Closing Price', color='blue')
    plt.title(f"{ticker} Stock Closing Prices ({start_date[:66]}-{end_date[:6]})")
    plt.xlabel("Date")
...
demo = gr.Interface(
    ...
    inputs=[
        ...
        gr.Textbox(label="End Date (YYYY-MM-DD)", value="2025-03-13")
    ],
